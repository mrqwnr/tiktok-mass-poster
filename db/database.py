...
class Database:
    ...
    def add_video(self, account_id: int, file_path: str, caption: str = "", hashtags: str = "") -> int:
        cur = self.conn.execute(
    ...
    def get_videos_for_account(self, account_id: int):
        return self.conn.execute(
            "SELECT * FROM videos WHERE account_id = ? ORDER BY id DESC", (account_id,)
        ).fetchall()
    ...
    def update_video_status(self, video_id: int, status: str, tiktok_video_id: str = ""):
        self.conn.execute(
            "UPDATE videos SET status = ?, tiktok_video_id = ?, posted_at = CURRENT_TIMESTAMP WHERE id = ?",
    ...
    def get_all_settings(self) -> dict:
        rows = self.conn.execute("SELECT key, value FROM global_settings").fetchall()
    ...
    def get_top_accounts(self, limit: int = 3) -> list:
